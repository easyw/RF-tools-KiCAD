...
##
def __ComputeCurved(vpercent, w, vec, pad, pts, segs,pDelta, nsx, nsy):
    ...
    # radius = via[1]/2
    radius = nsx/2 # pad.GetSize().x/2
    
    minVpercent = float(w*2) / float(nsx) # pad.GetSize().x) # via[1])
    ...
    vecC = pts[2] - pad.GetPosition() + pDelta# via[0]
    tangentC = [ pts[2][0] - vecC[1]*biasBC*weaken,
                 pts[2][1] + vecC[0]*biasBC*weaken ]
    vecE = pts[4] - pad.GetPosition() + pDelta# via[0]
...
def __ComputePoints(track, pad, segs):
    ...
    wxLogDebug('vec='+str(vec),dbg)
    # OK pDelta = wxPoint(int(vec[0]*-0.25*nsy), int(vec[1]*-0.25*nsy))
    #pDelta = wxPoint(int(vec[0]*-0.25*nsy), int(vec[1]*-0.25*nsy))
    pDelta = wxPoint(int(vec[0]*-0.25*nsy), int(vec[1]*-0.25*nsy))
    #pDelta = wxPoint(int(vec[0]*-0.05*nsy), int(0))
    #pDelta = wxPoint(int(sign(vec[0])*+0.25*nsy), int(sign(vec[1])*-0.25*nsy))
    #pDelta = wxPoint(int(x - sign(vec[0])*0.25*nsx), int(y - sign(vec[1])*0.25*nsy)) # wxPoint(int(vec[0]*-0.25*nsy), int(vec[1]*-0.25*nsy))
    #pDelta = wxPoint(int(sign(vec[0])*-0.25*nsx), int(sign(vec[1])*-0.25*nsy)) # wxPoint(int(vec[0]*-0.25*nsy), int(vec[1]*-0.25*nsy))
    
    #padCenterShift = wxPoint(int(x+sign(vec[0])*+0.25*nsx),int(y)) #+sign(vec[1])*+0.25*nsy))
    #pointC = padCenterShift + wxPoint(int(vecC[0] * radius), int(vecC[1] * radius)) - pDelta
    #pointE = padCenterShift + wxPoint(int(vecE[0] * radius), int(vecE[1] * radius)) - pDelta
    pointC = pad.GetPosition() + wxPoint(int(vecC[0] * radius), int(vecC[1] * radius)) - pDelta
    pointE = pad.GetPosition() + wxPoint(int(vecE[0] * radius), int(vecE[1] * radius)) - pDelta
    ...
    if segs > 2:
        pts = __ComputeCurved(vpercent, w, vecT, pad, pts, segs, pDelta, nsx, nsy)
        # Introduce a last point in order to cover the via centre.

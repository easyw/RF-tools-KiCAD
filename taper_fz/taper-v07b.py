...
##
def __ComputeCurved(vpercent, w, vec, pad, pts, segs, nsx, nsy):
...
def __ComputePoints(track, pad, segs):
    ...
    pointD = pad.GetPosition() + wxPoint(int(vec[0]*-0.12*radius), int(vec[1]*-0.12*radius))
...
def __ComputePointsTracks(track1, track2, segs):
    ...
    # Introduce a last point in order to cover the via centre.
    # If not, the zone won't be filled or not connected
    internal_delta_multiplier1 = 2.0
    idm1 = internal_delta_multiplier1
    
    #pointF = start1 + wxPoint( vecT1[0]*n -vecT1[1]*idm1*w1 , vecT1[1]*n -vecT1[0]*idm1*w1 )
    pointF = start1 + wxPoint(int(vecT1[0]*idm1*w1) , int(vecT1[1]*idm1*w1))
    ...
    internal_delta_multiplier2 = 2.0
    idm2 = internal_delta_multiplier2
    ...
    pointC2 = pointC + wxPoint(int(vecT2[0]*-idm2*w2), int(vecT2[1]*-idm2*w2))#  + wxPoint(int(vecT2[0]*-idm2*w1*shift2), int(vecT2[1]*-idm2*w1*shift2))
    pointE2 = pointE + wxPoint(int(vecT2[0]*-idm2*w2), int(vecT2[1]*-idm2*w2))#  + wxPoint(int(vecT2[0]*-idm2*w1*shift2), int(vecT2[1]*-idm2*w1*shift2))
    
    # Introduce a last point in order to cover the via centre.
    # If not, the zone won't be filled
    pointD = end2 + wxPoint(int(vecT2[0]*-idm2*w2), int(vecT2[1]*-idm2*w2)) + wxPoint(int(vecT2[0]*-idm2*w1*shift2), int(vecT2[1]*-idm2*w1*shift2))
    # pointD = via[0] + wxPoint(int(vec[0]*-0.5*radius), int(vec[1]*-0.5*radius))
...
def SetTaper_Zone(pcb=None):
    ...
    # square taper at the end of a track
        ...
        if result == wx.ID_OK:
            ...
            wx.LogMessage('Select:\none Track & one Pad\nor Two nearby Tracks, but not connected\nor\na single Track')
        dlg.Destroy()

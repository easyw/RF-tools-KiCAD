...
def __ComputePoints(track, pad, segs):
    ...
    modAng = module.GetOrientationDegrees()
    modAngRad = radians(modAng)
    wxLogDebug('Module Angle='+str(modAng),dbg)
    # if modAngRad > pi:
    #     modAngRad -=2*pi
    # if modAngRad < -pi:
    #     modAngRad +=2*pi
    # modAng=degrees(modAngRad)
    # wxLogDebug('Module Angle='+str(modAng),dbg)
    ...
    # we need to correlate module angle with track angle
    ...
    if ( __PointDistance(pointA, pad.GetPosition()) < radius or
        ...
        wxLogDebug('track too short ... aborting',True)
        return False
    ...
    wxLogDebug(' vecC[0]='+str(vecC[0])+' vecC[1]='+str(vecC[1]),dbg)
    wxLogDebug(' vecE[0]='+str(vecE[0])+' vecE[1]='+str(vecE[1]),dbg)
    ...
    wxLogDebug('cos(modAngRad)='+str(cos(modAngRad))+' sin(modAngRad)='+str(sin(modAngRad)),dbg)
    # shiftPc = wxPoint(int(vecC[0] * (+sx/2*cos(modAngRad) +sy/2*sin(modAngRad))), int(vecC[1] * (-sx/2*sin(modAngRad) -sy/2*cos(modAngRad)))) 
    # shiftPc = wxPoint(int(vecC[0] * (+sx/2*abs(cos(modAngRad)) )), int(vecC[1] * (+sy/2*abs(cos(modAngRad))))) 
    # shiftPe = wxPoint(int(vecE[0] * (-sx/2*abs(cos(modAngRad)) )), int(vecE[1] * (+sy/2*abs(cos(modAngRad)))))
    shiftPc = wxPoint(int(+vecC[0] * +(+sx/2*abs(cos(modAngRad))+sy/2*sin(modAngRad))), int(+vecC[1] * -(+sx/2*sin(modAngRad)-sy/2*abs(cos(modAngRad))))) 
    shiftPe = wxPoint(int(-vecE[0] * -(+sx/2*abs(cos(modAngRad))+sy/2*sin(modAngRad))), int(-vecE[1] * +(+sx/2*sin(modAngRad)-sy/2*abs(cos(modAngRad)))))
    # shiftPe = wxPoint(int(vecC[0] * (-sx/2*cos(modAngRad) -sy/2*sin(modAngRad))), int(vecC[1] * (+sx/2*sin(modAngRad) +sy/2*cos(modAngRad))))
    pointC = pad.GetPosition() +  shiftPc #radius)) # - wxPoint(int(vec[0]*-0.25*nsy), int(vec[1]*-0.25*nsy))
    pointE = pad.GetPosition() +  shiftPe 
    ...
    # shiftP = wxPoint(int(invx*signx*nsy*0.25), int(invy*signy*nsy*0.25))
    #shiftP2 = wxPoint(int(invx*signx*sy*0.25*sin(modAngRad)), int(invy*signy*sy*0.25*cos(modAngRad)))
    # shiftP2 = wxPoint(int(sx*0.25*(cos(modAngRad))*vecC[1]), int(sx*0.25*(sin(modAngRad))*vecC[0]))
    shiftP2c = wxPoint(int(+vecC[1] * -(+sx/2*sin(modAngRad)-sy/2*abs(cos(modAngRad)))),int(+vecC[0] * -(+sx/2*abs(cos(modAngRad))+sy/2*sin(modAngRad)))) 
    shiftP2e = wxPoint(int(+vecE[1] * +(+sx/2*sin(modAngRad)-sy/2*abs(cos(modAngRad)))),int(-vecE[0] * -(+sx/2*abs(cos(modAngRad))+sy/2*sin(modAngRad))))
    
    pointC2 = pointC + shiftP2c #wxPoint(int(invx*signx*nsy*0.25), int(invy*signy*nsy*0.25))
    pointE2 = pointE + shiftP2e #wxPoint(int(invx*signx*nsy*0.25), int(invy*signy*nsy*0.25))
    ...
    ## pts = [pointA, pointB, pointC2, pointD, pointE2]
    # pts = [pointA, pointF, pointB, pointC2, pointC, pointD, pointE, pointE2]
    #pts = [pointA, pointB, pointF, pointC, pointD, pointE]
    if segs > 2:
        #pts = __ComputeCurved(vpercent, w, vecT, pad, pts, segs, nsx, nsy)
        pts = [pointA, pointB, pointC2, pointD, pointE2]
        curve1, curve2 = __ComputeCurved(vpercent, w, vecT, pad, pts, segs, nsx, nsy, shiftD, shiftP2)
        # Introduce a last point in order to cover the via centre.
...
def SetTaper_Zone(pcb=None):
    ...
    # square taper at the end of a track
        ...
        RebuildAllZones(pcb)
    elif len(selTracks) == 0 and len(selPads) == 0:

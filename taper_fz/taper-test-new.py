...
def __ComputePoints(track, pad, segs):
    ...
    #     invx=0;invy=1
    mAng = module.GetOrientationDegrees()
    mAngRad = radians(mAng)
    ...
    elif abs(module.GetOrientationDegrees()) == 45 or abs(module.GetOrientationDegrees()) == 225:
        nsx = sy
        nsy  = sx
        wxLogDebug(' m3 '+'sx='+str(ToMM(sx))+' '+'sy='+str(ToMM(sy))+' '+'nsx='+str(ToMM(nsx))+' '+'nsy='+str(ToMM(nsy)),dbg)
        wxLogDebug(' m3 '+'mod angle='+str(module.GetOrientationDegrees()),dbg)    
    else:
        nsx = sy
        nsy  = sx
    ...
    if (abs(trackAngle) >= 45 and abs(trackAngle) <= 135) or (abs(trackAngle) >= 225 and abs(trackAngle) <= 315):
        ...
        #nsy  = nsx
        #invx=0;invy=1
        invx=abs(sin(mAngRad));invy=abs(cos(mAngRad))
        ...
        wxLogDebug(' t1 '+'invx=abs(sinMA)=>'+str(invx)+',invy=abs(cosMA)=> sinMA='+str(invy),dbg)
        wxLogDebug(' t1 '+'MA='+str(mAng),dbg)
    else:
        # nsx = nsx
        # nsy  = nsy
        #invx=1;invy=0
        invx=abs(cos(mAngRad));invy=abs(sin(mAngRad))
        wxLogDebug(' t2 '+'invx=abs(cosMA)=>'+str(invx)+',invy=abs(sinMA)=> sinMA='+str(invy),dbg)
        wxLogDebug(' t2 '+'MA='+str(mAng),dbg)

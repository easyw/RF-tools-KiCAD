...
def Taper_Zone(board): #, pad, track):
    ...
    if len(selTracks) == 1 and len(selPads) == 1:
        ...
        if horz:
            ...
            x2 = pts[1].x
            y2 = pts[1].y
            #points = [(x1 + l1/2*mr, y1 + w1/2), (x2 - l2/2*mr, y2 + w2/2), \
            #                                                                    (x2, y2 + w2/2), \
            #                                                                    (x2, y2 - w2/2),
            #                                   (x2 - l2/2*mr, y2 - w2/2), \
            #          (x1 + l1/2*mr, y1 - w1/2)]
            #wxLogDebug('points='+str(points),True)
            points = [(x1, y1 + w1/2), (x1 + l1/2*mr, y1 + w1/2), (x2 + - l2/2*mr, y2 + w2/2), (x2 + l2/2*mr, y2 + w2/2), \
                                                                                            (x2 + l2/2*mr, y2 - w2/2), \
                                                            (x2 - l2/2*mr, y2 - w2/2), \
                                    (x1 + l1/2*mr, y1 - w1/2), \
                    (x1, y1 - w1/2)]
       ...
       #  w1|<-----------|--|-M | w2   
       #    |            |  +   + 3           else: #vertical
       #    |            | / 4    x2,y2               w1 = pad.GetSize().x # FromMM(2.0)
       #    |            |/                        w2 = track.GetWidth() # FromMM(1.0)
       #    +            + 5                       l1 = pad.GetSize().y/2 # FromMM(1.6)
       #                                           l2 = track.GetWidth() # FromMM(0.6)
        else: #vertical

...
def __ComputePoints(track, pad, segs):
    ...
    invx=1;invy=1
    # use an angle range
    if abs(module.GetOrientationDegrees()) == 180 or abs(module.GetOrientationDegrees()) == 270:
        invx=-1
    ...
    if segs > 2:
        ...
        curve1, curve2 = __ComputeCurved(vpercent, w, vecT, pad, pts, segs, nsx, nsy)
        # Introduce a last point in order to cover the via centre.
        # if not it may not be connected
        #pts = [pointF]+curve1+[pointC,pointD,pointE]+curve2
        pts = [pointF]+curve1+[pointC2,pointD,pointE2]+curve2

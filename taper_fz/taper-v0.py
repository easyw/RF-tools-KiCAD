...
from pcbnew import ToMM, TRACK, FromMM, wxPoint, GetBoard, ZONE_CONTAINER
...
#     #     pass
# 
#     def Taper_FZ(self,board): # , center=pcbnew.wxPoint(0,0), angle_deg=0, board):
#         pass
         #self.ZONE_CONTAINER(board) = ZONE_CONTAINER(board)
         ...
         # 
         # # Add zone properties
         # self.SetLayer(track.GetLayer())
         # self.SetNetCode(track.GetNetCode())
         # self.SetZoneClearance(track.GetClearance())
         # self.SetMinThickness(25400)  # The minimum
         # self.SetPadConnection(2)  # 2 -> solid
         # self.SetIsFilled(True)
         # self.SetPriority(MAGIC_TAPER_ZONE_ID)
         # ol = self.Outline()
         # ol.NewOutline()
         # 
         # for p in points:
...
def Taper_Zone(board): #, pad, track):
    ...
    if len(selTracks) == 1 and len(selPads) == 1:
       ...
       #                 6
                   ...
                   #              6,
                   # 7
        points = [(x1, y1 + w1/2), (x1 + l1*mr, y + w1/2), (x2 + - l2/2*mr, y2 + w2/2), (x2 + l2/2*mr, y2 + w2/2), \
                                                                                        (x2 + l2/2*mr, y2 - w2/2), \
                                                        (x2 - l2/2*mr, y2 - w2/2), \
                                 (x1 + l1*mr, y1 - w1/2), \
                  (x1, y1 - w1/2)]
...
class Layout:
    ...
    @staticmethod
    def get_tracks_by_pos(pnt, board=None):
        if board is None:
            board = pcbnew.GetBoard()
        tks = board.GetTracksByPosition(pnt)
        return list(filter(lambda t: t.GetStart(), board.GetTracksByPosition(pnt)))
